...
class DatasetLoader():
    """Load images, segmentations and ranks.
    ...
    - sequence_dir
        ...
        - jaccard_ranks_001.json
        - jaccard_ranks_002.json
                ...
        - jaccard_ranks_NNN.json
    ...
    def _single_load(self, _dir):
        ...
        seg_list, image_list, ranks_list = self._image_seg_pairs(_dir)
        seg_files = np.ndarray([len(seg_list), self.image_size[0], self.image_size[1], self.channels])

        image_files = np.ndarray([len(image_list), self.image_size[0], self.image_size[1], self.channels])
        idx = 0
        for seg in seg_list:
            if os.path.splitext(seg)[1] == '.tiff':
                seg_files[idx, :, :, :] = np.expand_dims(tiff.imread(seg), axis=2)
        ...
        for image in image_list:
            if os.path.splitext(image)[1] == '.tiff':
                image_files[idx, :, :, :] = np.expand_dims(tiff.imread(image), axis=2)
            elif self.channels == 3:
                pass
            else:
                pass
        ...
        seg_tensor = tf.convert_to_tensor(seg_files, dtype=np.float32)
        ranks_tensor = tf.convert_to_tensor(ranks_list, dtype=np.float32)
        # ranks_tensor = tf.reshape(ranks_tensor, [-1, 1])

        return image_tensor, seg_tensor, ranks_tensor
    ...
    def _image_seg_pairs(self, datadir):
        seg_list = []
        image_list = []
        image_names = []
        ranks_list = []
        if self.params["rank_type"] == 'dice':
            isdice = True
        else:
            isdice = False

        # load image names into a list
        imagedir = (self.directory + '/%s/image' % datadir)
        # TODO think if we don't want to get the absolute dir
        # in the function call (load function in origin)
        image_filenames = os.listdir(imagedir)
        for image in image_filenames:
            if not np.any(os.path.splitext(image)[1] == self.params["standard_file_type"]):
                continue
            image_list.append((imagedir + '/' + image))
            image_names.append(image)

        # try loading a seg dir until we reached the last
        try:
            segdiridx = 1
            while True:
                ranks = self._ranks_reader(datadir, segdiridx, isdice)
                ranks_list.append(ranks)
                segdir = (self.directory + '/%s/seg_%03.f' % (datadir, segdiridx))
                seg_files = os.listdir(segdir)
                for segidx, segname in enumerate(seg_files):
                    if not np.any(os.path.splitext(segname)[1] == self.params["standard_file_type"]):
                        continue
                    # TODO change seg_file_idx to be taken from file name
                    # if filename of current seg file is different from the corresponding image loaded

                    if not segname == image_names[segidx]:
                        error_msg = "no corresponding seg in segdir %s for image %s" \
                            % (segdir, image_list[segidx])
                        raise ValueError(error_msg)  # TODO is NameError the right error?
                    seg_list.append(segdir + '/' + segname)
                segdiridx += 1
        except (FileNotFoundError, NameError) as err:
            if segdiridx == 1:
                raise err
            print('Dataloader: Loaded ' + str(segdiridx) + ' Segmentation Directories')
        return seg_list, image_list, ranks_list
    ...
    def tile_load(self, dir_list):
        ...
        images, segs, ranks = self._single_load(dir_list[0])
        num_images = images.shape.as_list()[0]
        num_segs = segs.shape.as_list()[0]

        images = tf.tile(images, [np.int32(num_segs / num_images), 1, 1, 1])
        ranks = tf.reshape(ranks, [-1])
        # will raise exception if number of images != number of segs
        return tf.data.Dataset.from_tensor_slices((images, segs, ranks))
    ...
    def input_fn(dataset):
        ...
        def parser(images, segs, ranks):
            features = {"images": images, "segs": segs}
            labels = ranks
