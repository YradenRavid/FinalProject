...
def main(argv):
    ...
    if train:
        dataset = loader.tile_load(datadirs[0])  # should load all image-seg pairs
        dataset = loader.shuffle_batch_repeat(dataset)
        # Dirs assumed model is GenDir / SeqDir / Image or Segs or GT dir
        for (idx, _dir) in enumerate(datadirs):
            ...
            try:
                ...
                if idx == 0:
                    continue
                dataset = loader.tile_load(_dir)  # should load all image-seg pairs
                dataset = loader.shuffle_batch_repeat(dataset)
